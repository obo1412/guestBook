...
public class GuestBookController {
	...
	@RequestMapping(value= {"/event_make.do"})
	public ModelAndView eventMake(Locale locale, Model model,
			HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		web.init();
		
		String nameEvent = web.getString("event-name");
		
		logger.debug(nameEvent);
		
		GuestBook data = new GuestBook();
		
		data.setNameEvent(nameEvent);
